...
# Helper functions
log_info() { 
    echo -e "${BLUE}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} [INFO] $*" | tee -a "$SCAN_LOG"
}

log_warn() { 
    echo -e "${YELLOW}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} [WARN] $*" | tee -a "$SCAN_LOG"
}

log_success() { 
    echo -e "${GREEN}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} [SUCCESS] $*" | tee -a "$SCAN_LOG"
}

log_error() { 
    echo -e "${RED}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} [ERROR] $*" | tee -a "$SCAN_LOG" "$ALERT_LOG"
}

log_critical() {
    echo -e "${MAGENTA}[$(date '+%Y-%m-%d %H:%M:%S')]${NC} [CRITICAL] $*" | tee -a "$SCAN_LOG" "$ALERT_LOG"
}
    ...
    for web_dir in "${WEB_DIRS[@]}"; do
        for dir in $web_dir; do
            if [[ -d "$dir" ]]; then
                ...
                find "$dir" -type f -name "*.php" -print0 2>/dev/null | while IFS= read -r -d '' php_file; do
